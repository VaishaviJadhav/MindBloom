...
const styles = `
  ...
  * { cursor: none !important; }
  ...
  .xhair-ring { position:absolute; width:28px; height:28px; border-radius:50%;
    border:2px solid rgba(245,230,66,0.7); transform:translate(-50%,-50%); pointer-events:none; }
...
function GunSVG() {
  return (
    <svg width="130" height="70" viewBox="0 0 130 70" style={{ display: "block" }}>
      ...
      {[37,41,45,49].map(x => <line key={x} x1={x} y1="15" x2={x} y2="31" stroke="#777" strokeWidth="1.8" />)}
      ...
      {[37,41,45,49,53].map(y => <line key={y} x1="41" y1={y} x2="58" y2={y} stroke="#6b4a12" strokeWidth="1.2" />)}
...
/* ══════════════════════════════════════════════════════
   GENERIC LEVEL COMPONENT  (Levels 2–7)

   Props:
     levelNumber — 2..7
     words       — [{ word, options, level }] from server
     childId     — child UUID
     onComplete  — (results) => void   ← called by orchestrator
                   results = [{ word, selected, correct, level }]

   ⚠️  This component does NOT manage daily scores.
       Daily score save lives in SoundSafariGame (orchestrator).
       This component only saves: game_sessions, game_attempts,
       predictions, child_progress.
══════════════════════════════════════════════════════ */
export default function SoundSafariLevel({ levelNumber, words, childId, onComplete }) {
  ...
  useEffect(() => {
    const onDown = () => {
      ...
      if (idx === null || !currentWordRef.current) return;

      reactionTimesRef.current = [...reactionTimesRef.current, Date.now() - questionStartTimeRef.current];
  ...
  /* ══════════════════════════════════════════════════════
     Save game_sessions, game_attempts, predictions,
     child_progress — but NOT child_daily_scores
     (that is handled by SoundSafariGame orchestrator)
  ══════════════════════════════════════════════════════ */
  const saveToSupabase = async (r, mlResult) => {
    const cId = childData?.id;
    if (!cId) { console.error("❌ No child_id"); return; }
    ...
    const wCorrect        = r.reduce((s, x) => s + (x.correct ? (weights[x.level] || 1) : 0), 0);
    const wTotal          = r.reduce((s, x) => s + (weights[x.level] || 1), 0);
    ...
    const avgReactionTime = times.length > 0
      ? parseFloat((times.reduce((a, b) => a + b, 0) / times.length / 1000).toFixed(3)) : null;
    ...
    await supabase.from("levels").upsert(
      { id: levelNumber, sound_difficulty: "adaptive", mirror_difficulty: "easy", memory_card_count: 4 },
      { onConflict: "id" }
    );
    ...
    const { data: sessionData, error: sessionErr } = await supabase
      .from("game_sessions")
      .insert({ child_id: cId, level_played: levelNumber, overall_score: overallScore, avg_response_time: avgReactionTime })
      .select("id").single();
    if (sessionErr) { console.error("❌ game_sessions:", sessionErr.message); return; }

    // game_attempts
    await supabase.from("game_attempts").insert({
      session_id: sessionData.id, game_type: "sound",
      total_questions: totalQuestions, correct_answers: correctAnswers,
      wrong_answers: wrongAnswers, accuracy, avg_response_time: avgReactionTime,
    });

    // predictions
    await supabase.from("predictions").insert({
      child_id: cId, phoneme_score: phonoScore,
      predicted_level: predictedLevel, confidence_score: parseFloat(mlResult?.confidence) || null,
    });

    // child_progress
    const { data: existing } = await supabase.from("child_progress").select("id").eq("child_id", cId).maybeSingle();
    if (existing) {
      await supabase.from("child_progress")
        .update({ current_level: levelNumber, last_score: overallScore, updated_at: new Date().toISOString() })
        .eq("child_id", cId);
    } else {
      await supabase.from("child_progress").insert({ child_id: cId, current_level: levelNumber, last_score: overallScore });
    }

    console.log(`✅ Level ${levelNumber} saved to Supabase`);
  };

  /* ── ML submit ── */
  const submitToMLService = async () => {
    if (!childData) { alert("Child data not loaded."); return; }
    setIsLoadingML(true);
    const r = gameResultsRef.current;
    const byLevel = (lvl, chk) => r.filter(x => x.level === lvl && (chk === undefined || x.correct === chk));
    ...
    const mlData = {
      ...
      Clicks1:  byLevel("easy").length,        Hits1:  byLevel("easy",     true).length,
      Misses1:  byLevel("easy",false).length,  Score1: byLevel("easy",     true).length * 10,
      Accuracy1: byLevel("easy",  true).length / (byLevel("easy").length  || 1),
      Missrate1: byLevel("easy", false).length / (byLevel("easy").length  || 1),
      Clicks2:  byLevel("moderate").length,    Hits2:  byLevel("moderate", true).length,
      Misses2:  byLevel("moderate",false).length, Score2: byLevel("moderate", true).length * 10,
      ...
      Clicks3:  byLevel("hard").length,        Hits3:  byLevel("hard",     true).length,
      Misses3:  byLevel("hard", false).length, Score3: byLevel("hard",     true).length * 10,
    ...
    try {
      const res = await fetch("/predict", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(mlData),
      });
      if (!res.ok) throw new Error(`${res.status}: ${await res.text()}`);
      const mlResult = await res.json();

      await saveToSupabase(r, mlResult);

      await Swal.fire({
        title: levelNumber === 7 ? "🏆 All Levels Complete!" : "🎉 Level Complete!",
        html:  levelNumber === 7
          ? `<b>Amazing work!</b><br/>You've completed all 7 levels!`
          : `<b>Level ${levelNumber} done!</b><br/>Get ready for Level ${levelNumber + 1}!`,
        icon: "success",
        confirmButtonText: levelNumber === 7 ? "See Results" : `Start Level ${levelNumber + 1}`,
      });

      // ← Pass results to SoundSafariGame orchestrator
      // Orchestrator handles daily score save
      onComplete?.(r);

    } catch (err) {
      console.error("❌ ML submit failed:", err);
      alert(`ML service error: ${err.message}`);
    } finally {
      setIsLoadingML(false);
    }
  };

  /* ── Next word / finish level ── */
  ...
  if (!currentWord) return <div style={{ color: "white", padding: 40 }}>Loading Level {levelNumber}...</div>;
  ...
  return (
    <>
      ...
      <div className="safari-root">
        <span className="leaf leaf-tl">🌿</span><span className="leaf leaf-tr">🌿</span>
        <span className="leaf leaf-bl">🍃</span><span className="leaf leaf-br">🍃</span>
        <span className="leaf leaf-ml">🌱</span><span className="leaf leaf-mr">🌱</span>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{
              ...
              background: "rgba(168,230,61,0.15)", border: "1px solid rgba(168,230,61,0.3)",
              fontFamily: "'Fredoka One', cursive", fontSize: "0.8rem", color: "#a8e63d", letterSpacing: 1,
            }}>
              LEVEL {levelNumber}
            </span>
          </div>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <div className="vine-track"><div className="vine-fill" style={{ width: `${progress}%` }} /></div>
        </div>
        ...
        <div style={{ textAlign: "center", marginBottom: 10 }}>
          <p style={{ color: "#8dc87a", fontSize: "0.88rem", margin: "0 0 12px" }}>
            🎧 Listen — move your <strong style={{ color: "#f5e642" }}>crosshair</strong> over the correct spelling and{" "}
            <strong style={{ color: "#f5e642" }}>click</strong> to shoot!
          </p>
          <button className="play-btn" onClick={() => {
            window.speechSynthesis.cancel();
            setTimeout(() => {
              const u = new SpeechSynthesisUtterance(currentWord.word);
              u.rate = 0.5; u.pitch = 1.2; window.speechSynthesis.speak(u);
            }, 120);
          }}>🔊 Play Again</button>
        </div>
        ...
        <div className="options-grid">
          {currentWord.options.map((o, i) => {
            ...
            return (
              <button key={o} ref={el => btnRefs.current[i] = el}
                className={`option-btn ${cls}`}
                style={{ "--opt-dur": OPT_FLOATS[i].dur, "--opt-dly": OPT_FLOATS[i].dly }}
                disabled={showFeedback}>
                {o}
                {bangIdx === i && (
                  <span className="bang-burst">{isCorrect ? "💥 BANG!" : "💨 MISS!"}</span>
                )}
              </button>
            );
          })}
        ...
        {showFeedback && (
          <div className="feedback-banner">
            ...
            <button className="next-btn" onClick={nextWord} disabled={isLoadingML}>
              {isLoadingML ? "🔬 Analyzing…" : isLast ? `🏁 Finish Level ${levelNumber}` : "Next Target →"}
            </button>
          </div>
        )}
